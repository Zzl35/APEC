...
class DACAgent:
    ...
    def update_actor(self, obs, bc_regularize, step):
        metrics = dict()
        ...
        if bc_regularize:
            with torch.no_grad():
                ckpt_dist = self.bc_actor.actor(obs, stddev)
                ckpt_action = ckpt_dist.sample(clip=self.stddev_clip)
            # ckpt_output = torch.from_numpy(ckpt_output).to(self.device)
            bc_loss = torch.square(ckpt_action - action).mean()
            actor_loss += bc_loss * bc_weight
    ...
    def update(self, replay_iter, expert_replay_iter, step, bc_regularize=False):
        ...
        # update actor
        metrics.update(self.update_actor(obs.detach(), bc_regularize, step))
    ...
    def dac_rewarder(self, obses, actions, reward_model):
