...
class DrQv2Agent:
    ...
    def update_actor(self, obs, obs_qfilter, action_bc, bc_regularize, step):
        metrics = dict()
        ...
        if bc_regularize:
            log_prob_bc = dist.log_prob(action_bc).sum(-1, keepdim=True)
            actor_loss += -log_prob_bc.mean()*bc_weight*0.01
        ...
        if self.use_tb:
            ...
            if bc_regularize:
                metrics['regularized_bc_loss'] = - log_prob_bc.mean().item()*bc_weight*0.01
                metrics['bc_loss'] = - log_prob_bc.mean().item()*0.01
    ...
    def update(self, replay_iter, expert_replay_iter, step, bc_regularize=False):
        ...
        if bc_regularize:
            ...
            if bc_regularize and self.bc_weight_type=="qfilter":
                ...
                obs_qfilter = None
            dist_bc = self.actor_bc(obs_qfilter, 0.1)
            action_bc = dist_bc.mean
        else:
            obs_qfilter = None
            action_bc = None
        ...
        # update actor
        metrics.update(self.update_actor(obs.detach(), obs_qfilter, action_bc, bc_regularize, step))
