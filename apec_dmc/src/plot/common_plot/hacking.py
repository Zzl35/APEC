...
def create_subplots(num_plots):
    ...
    axest = [ax.twinx() for ax in axes]
    return fig, axes, cols,rows, axest
...
def visual_hacking(methods_mjc, methods_dmc, savefig=True, savepath=None):
    FONTSIZE = 27
    ...
    real_range = {
        'AntFH-v0':(-300, 6000),
        'HalfCheetahFH-v0': (-500, 15000),
        'HopperFH-v0':(0, 4000),
        'HumanoidFH-v0':(-100, 5500),
        'Walker2dFH-v0':(-500, 5500),
        'cheetah_run':(-100, 1000),
        'walker_run':(-100, 1000),
        'walker_walk':(-100, 1000)
    }

    fig, axes, ncols, nrows, axest = create_subplots(len(env_names_mjc + env_names_dmc))
    for i, key in enumerate(methods_dmc.keys()):
        ...
        for j, env_name in enumerate(env_names_mjc):
            ...
            curves_real, curves_pred = [], []
            real_min, real_max = real_range[env_name]
            if os.path.exists(logdir):
                for dirname in os.listdir(logdir):
                    ...
                    df['Episode Return'] = (curve_smoothed - real_min) / (real_max - real_min)
                    curves_real.append(df)

                    if 'sac/test_eval_reward' not in ea.Tags()['scalars']:
                        print(f'Tensorboard@{tb_dir} not contains sac/test_eval_reward, skipped.')
                        continue
                    training_curve = ea.scalars.Items('sac/test_eval_reward')
                    curve = np.array([dot.value for dot in training_curve])[:timesteps]
                    curve_resized = resize_curve(curve, new_length=timesteps)
                    curve_smoothed = gaussian_filter1d(curve_resized, sigma=5)
                    df = pd.DataFrame()
                    df['Training Iteration'] = np.arange(len(curve_smoothed))
                    df['Episode Return'] = (curve_smoothed - curve_smoothed.min()) / (curve_smoothed.max() - curve_smoothed.min())
                    curves_pred.append(df)
            else:
                print(f'Logfile@{logdir} not exists, skipped.')
                continue
            curves_real = pd.concat(curves_real, axis=0, ignore_index=True)
            curves_pred = pd.concat(curves_pred, axis=0, ignore_index=True)
            sns.lineplot(curves_real, x='Training Iteration', y='Episode Return', ax=axes[j], label=key, color=COLORS[1], errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=4)
            sns.lineplot(curves_pred, x='Training Iteration', y='Episode Return', ax=axest[j], label=key, color=COLORS[5], errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=4)
        ...
        for j, env_name in enumerate(env_names_dmc):
            ...
            if os.path.exists(logdir):
                for dirname in os.listdir(logdir):
                    ...
                    if 'eval/episode_reward_our' not in ea.Tags()['scalars']:
                        print(f'Tensorboard@{tb_dir} not contains eval/episode_reward_our, skipped.')
                        continue
                    training_curve = ea.scalars.Items('eval/episode_reward_our')
                    curve = np.array([dot.value for dot in training_curve])[:timesteps]
                    curve_resized = resize_curve(curve, new_length=timesteps)
                    curve_smoothed = gaussian_filter1d(curve_resized, sigma=0.5)
                    df = pd.DataFrame()
            ...
            curves_real = pd.concat(curves_real, axis=0, ignore_index=True)
            curves_pred = pd.concat(curves_pred, axis=0, ignore_index=True)
            sns.lineplot(curves_real, x='Training Iteration', y='Episode Return', ax=axes[new_j], label=key, color=COLORS[1], errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=4)
            sns.lineplot(curves_pred, x='Training Iteration', y='Episode Return', ax=axest[new_j], label=key, color=COLORS[5], errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=4)
            print(f'{env_name}-{method}-done.')
        ...
        for i, env_name in enumerate(env_names_mjc+env_names_dmc):
            ...
            axes[i].set_ylim(0, 1)
            # axest[i].set_ylim(0, 1)
            axes[i].tick_params(labelsize=FONTSIZE-5)
            axest[i].tick_params(labelsize=FONTSIZE-5)
            # if(i % ncols == 0):
            #     axes[i].set_ylabel("Normalized Episode Return", fontsize=FONTSIZE+3)
            # else:
            #     axes[i].set_ylabel('', fontsize=FONTSIZE+1)
            axes[i].set_ylabel('', fontsize=FONTSIZE+1)
            axest[i].set_ylabel('', fontsize=FONTSIZE+1)

    plt.tight_layout()
    handles = [mlines.Line2D([], [], color=COLORS[1], label='Real episode return', linewidth=5),
               mlines.Line2D([], [], color=COLORS[5], label='Predicted episode return', linewidth=5)]
    # handles.insert(0, mlines.Line2D([], [], color=COLORS[0], label='Demos.(Avg)', linewidth=5))
...
if __name__ == '__main__':
    ...
    for m in methods_dmc.keys():
        mjc = {m:methods_mjc[m]}
        dmc = {m:methods_dmc[m]}
        visual_hacking(mjc, dmc,
                       savepath=f'exp_local/plot_output/reward_hacking/{m}.pdf')
