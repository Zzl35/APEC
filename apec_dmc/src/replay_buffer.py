...
class ReplayBufferStorage:
    def __init__(self, data_specs, replay_dir, load_dir=None):
        self._data_specs = data_specs
        self._replay_dir = replay_dir
        replay_dir.mkdir(exist_ok=True)
        self._current_episode = defaultdict(list)
        self._preload(load_dir)
    ...
    def _preload(self, load_dir=None):
        self._num_episodes = 0
        self._num_transitions = 0
        if load_dir is None:
            load_dir = self._replay_dir
        for fn in load_dir.glob('*.npz'):
            target_fn = os.path.join(self._replay_dir, str(fn).split('/')[-1])
            if str(fn) != target_fn:
                shutil.copy(fn, target_fn)
            # print(f'{target_fn} preloaded')
