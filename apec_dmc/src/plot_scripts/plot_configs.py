...
method2label = {
    'ablation_nonoise': 'APEC(ours)',
    'irl': 'AIL',
    # 'ablation_noepoch':'Ours (Last Iter)',
    'ablation_4epoch': 'APEC (with few checkpoints)',
    'ablation_40epoch': 'APEC (40 checkpoints)',
    'ablation_sample_nodistance': 'APEC (iteration guidance only)',
    'ablation_sample_noiter': 'APEC (OT distance guidance only)',
    'ablation_sample_relative': 'APEC (Relative distance guidance)',
...
model_hypeparams = {
    ...
    'baseline_ssrr':[       'pbrl',     'baseline_ssrr',    'baseline_ssrr'],           # 自回归sigmoid，再用reward逼近sigmoid
...
COLORS = ['grey', 'green', 'darkorange', 'firebrick', '#a65628', 'blue']
...
# tot_df = pd.concat([dac_df,], axis=0, ignore_index=True)
        # sns.set_theme(style='darkgrid')
        # sns.lineplot(data=eail_df, ax=ax, x='Steps', y='Episode Return', label="OPT-AIL (Ours)", color='blue', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=3)
        # # sns.lineplot(data=dac_df, ax=ax, x='Steps', y='Episode Return', label="DAC", color='purple', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=3)
        # sns.lineplot(data=iqlearn_df, ax=ax, x='Steps', y='Episode Return', label="IQLearn", color='darkorange', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=3)
        # sns.lineplot(data=ppil_df, ax=ax, x='Steps', y='Episode Return', label="PPIL", color='firebrick', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=3)
        # sns.lineplot(data=filter_df, ax=ax, x='Steps', y='Episode Return', label="FILTER", color='#a65628', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=3)
        # sns.lineplot(data=hype_df, ax=ax, x='Steps', y='Episode Return', label="HyPE", color='purple', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=3)
        # sns.lineplot(data=expert_df, ax=ax, x='Steps', y='Episode Return', label="Expert", color='grey', linestyle='dashed', errorbar=None, legend=False, linewidth=3)
        # sns.lineplot(data=bc_df, ax=ax, x='Steps', y='Episode Return', label="BC", color='green', linestyle='dashed', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=3)