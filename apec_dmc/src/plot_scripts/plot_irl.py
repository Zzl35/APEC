...
def visual_reusability(methods_mjc, methods_dmc, savefig=True, savepath=None):
    FONTSIZE = 27
    env_names_mjc = ['HumanoidFH-v0']
    env_names_dmc = ['walker_run']
    ...
    for j, env_name in enumerate(env_names_mjc):
        ...
        axes[j].plot(range(timesteps), [expert_return for _ in range(timesteps)], label='demos', linestyle='--', color='gray', linewidth=5)

        for i, key in enumerate(methods_mjc.keys()):
            method = methods_mjc[key]
            logdir = os.path.join('/home/ubuntu/duxinghao/imitation_pref/logfile', 'train_irl', method, ENV2ENV_NAME[env_name], '0_0_1_0')
            curves = []
            if os.path.exists(logdir):
                for dirname in os.listdir(logdir):
                    ...
                    ea.Reload()
                    if 'tb/StoEpRetTest' not in ea.Tags()['scalars']:
                        print(f'Tensorboard@{tb_dir} not contains tb/StoEpRetTest, skipped.')
                        continue
                    training_curve = ea.scalars.Items('tb/StoEpRetTest')
            ...
            sns.lineplot(curves, x='Training Iteration', y='Episode Return', ax=axes[j], label=key, color='blue', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=4)
    ...
    for j, env_name in enumerate(env_names_dmc):
        ...
        axes[j+len(env_names_mjc)].plot(range(timesteps), [expert_reward_mean for _ in range(timesteps)], label='demos', linestyle='--', color='gray', linewidth=5)

        for i, key in enumerate(methods_dmc.keys()):
            method = methods_dmc[key]
            logdir = os.path.join('exp_local', f'train_irl', 'dmc', env_name)
            curves = []
            if os.path.exists(logdir):
                for dirname in os.listdir(logdir):
                    tb_dir = os.path.join(logdir, dirname, 'suboptimal', 'tb')
                    ...
                    ea = event_accumulator.EventAccumulator(tb_path)
                    ea.Reload()
                    if 'eval/episode_reward' not in ea.Tags()['scalars']:
                        print(f'Tensorboard@{tb_dir} not contains eval/episode_reward, skipped.')
                        continue
                    training_curve = ea.scalars.Items('eval/episode_reward')
                    curve = np.array([dot.value for dot in training_curve])[:timesteps]
                    curve_resized = resize_curve(curve, new_length=timesteps)
                ...
                continue
            curves = pd.concat(curves, axis=0, ignore_index=True)
            sns.lineplot(curves, x='Training Iteration', y='Episode Return', ax=axes[new_j], label=key, color='blue', errorbar='sd', err_kws={"alpha": 0.1}, legend=False, linewidth=4)
            print(f'{env_name}-{method}-done.')
    ...
    for i, env_name in enumerate(env_names_mjc+env_names_dmc):
        ...
        if env_name in env_names_dmc:
            ...
            axes[i].set(xlabel=r"Training Frames ($\times 10^4$)", xticks=[0, 10, 20, 30, 40, 50], xticklabels=[0, 20, 40, 60, 80, 100])
        else:
            axes[i].set_xlabel('', fontsize=FONTSIZE+3)
            axes[i].set(xlabel=r"Training Steps ($\times 10^4$)", xticks=[0, 50, 100, 150, 200, 250], xticklabels=[0, 5, 10, 15, 20, 25])
    ...
    plt.tight_layout()
    handles = [mlines.Line2D([], [], color='blue', label=method, linewidth=5) for i, method in enumerate(methods_mjc.keys())]
...
if __name__ == '__main__':
    ...
    methods_mjc = {method2label[method]:method for method in ['irl']}

    visual_reusability(methods_mjc, methods_dmc,
                       savepath='exp_local/plot_output/irl.pdf')
