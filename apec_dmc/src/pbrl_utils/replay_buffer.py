...
from pbrl_utils.epsilon_utils import NoiseInjectedPolicy, gen_traj, MLPBC
...
def epsilon_sample(file_path, expert_states, expert_actions, max_ep_len, sample_num, segment_len, sub_level=8, mode="train", task_type='baseline_drex'):
    ...
    with open(file_path, 'rb') as f:
        trajs = pickle.load(f)
    D = []
    min_noise_margin = 0.3 # from drex paper
    ...
    with tqdm(total=sample_num) as pbar:
        ...
        for _ in range(sample_num):
            ...
            if task_type in ['baseline_lerp', 'baseline_ssrr', 'baseline_ssrr_irl', 'baseline_ssrr_bc']:
                preference_dataset.add((states_1, states_2, actions_1, actions_2, rewards_1, rewards_2, masks_1, masks_2, pref_noise))
            else:
                preference_dataset.add((states_1, states_2, actions_1, actions_2, rewards_1, rewards_2, masks_1, masks_2, pref_label))
...
def collect_epsilon_dataset(file_paths, env, parameter, device, save_path):
    ...
    '''epsilon sampling'''
    num_samples_total = 0
    noise_range_i = np.arange(0, 1, 0.1)
...
def make_preference_dataset_no_noise(file_path, expert_states, expert_actions, max_ep_len, sample_num, segment_len, sub_level=8, mode="train", num_epochs=400, filter_distance=True, distance_threshold=0.1, filter_epoch=True, epoch_threshold=10):
    '''
    ...
    trajs = []
    epoch_max = 400 # need to change if epoch_total changed
