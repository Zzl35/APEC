...
class WorkspaceIL:
    def __init__(self, cfg, work_dir):
        ...
        print(f'load expert from {self.cfg.expert_dataset}')
        ...
        self.expert_reward_std = np.std(self.expert_reward[:self.cfg.num_demos].sum(axis=1))
        self.expert_reward = np.mean(self.expert_reward[:self.cfg.num_demos].sum(axis=1))
        print(f'expert return mean: {self.expert_reward}   std: {self.expert_reward_std}')
        
    def setup(self):
        # create logger
        self.logger = Logger(self.work_dir, use_tb=self.cfg.use_tb, task_type='train_irl')
        # create envs
    ...
    def eval(self):
        step, episode, total_reward, reward_max, reward_min = 0, 0, 0, 0, 1000
        ...
        while eval_until_episode(episode):
            ...
            reward_min = min(total_reward, reward_min)
            reward_max = max(total_reward, reward_max)
            episode += 1
        ...
        with self.logger.log_and_dump_ctx(self.global_frame, ty='eval') as log:
            ...
            log('episode_reward_max', reward_max)
            log('episode_reward_min', reward_min)
    ...
    def train_il(self):
        ...
        while train_until_step(self.global_step):
            if time_step.last():
                ...
                if metrics is not None and self.global_episode % int(1e4) == 0:
                    # log stats
            ...
            if not seed_until_step(self.global_step):
                ...
                if self.global_episode % int(1e4) == 0:
...
@hydra.main(config_path='../cfgs', config_name='config_train')
def main(cfg):
    workspace = WorkspaceIL(cfg, work_dir=f'train_irl/{cfg.suite.name}_{cfg.num_demos}/{cfg.task_name}/{cfg.seed}/{cfg.buffer_type}')
    workspace.train_il()
